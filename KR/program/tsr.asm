...
;
; Примечания:
;  1) комментарии, начинающиеся с символа @ - места, где код зависит от варианта
;
; Авторы:
...
code segment	'code'
 ...
 ; Игнорирование
	...
	cursiveSymbol 				DB 00000000b
						;@ символ И, составленный из единичек (его курсивный вариант)
								DB 00000000b
								DB 01100011b
								DB 01100111b
								DB 01100111b
...
; Справка
	helpMsg DB '>tsr.com [/?] [/u]', 10, 13
			...
			DB '  F2  - включение и отключения курсивного вывода русского символа В', 10, 13
			DB '  F3  - включение и отключение частичной русификации клавиатуры(F<DUL -> АБВГД)', 10, 13
			DB '  f9  - включение и отключение режима блокировки ввода латинских строчных букв', 10, 13
